...
- **TTL** — Technitium always stores a TTL. Entries with an empty (automatic) TTL are pushed with `3600`, and records at `3600` are treated as automatic in drift comparison. If you want a TTL that is compared exactly, set any explicit value other than 3600.
