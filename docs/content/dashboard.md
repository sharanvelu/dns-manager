...
| **Providers healthy** | Shown as `X/Y`: providers whose most recent connectivity/drift check succeeded, out of all configured providers. |
...
- **Health badge**
  - **Healthy** — the last drift check completed successfully (the provider was reachable and records were listed).
...
The timestamp on each card is the time of the last drift check.
