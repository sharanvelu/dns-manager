...
- **Sidebar is persona-driven** (`app-sidebar.tsx`, gated on `auth.can`; empty sections render nothing):
  ...
  The Settings group is a **collapsible `NavGroup`** (`nav-group.tsx`): `Collapsible` wrapping a `SidebarGroup`, the group label as `CollapsibleTrigger` (pointer cursor) with a `ChevronRight` that rotates 90° when open (`group-data-[state=open]/collapsible:rotate-90`), default open, same prefix-based active matching as `NavMain`. When the sidebar is icon-collapsed the group is **forced open and its trigger is `pointer-events-none`** — the label is visually hidden there (`opacity-0`, pulled up over the item above), so a clickable trigger would swallow clicks meant for the last main-nav item. Reuse it for any future grouped nav section.
...
## Docs surface (docs-site/)

The documentation is a single Next.js site (`docs-site/`) serving `/` (product landing) and `/docs` (documentation), baked into the Docker image at `/docs` and deployed to Vercel. **It deliberately does NOT follow this file's app design language** (user decision, 2026-07-29) — it is treated as its own product with a bolder, gradient-rich identity. Its conventions live in the site itself:

- **Pluggable palette (rule)**: every docs accent color flows through the semantic tokens in `docs-site/app/palette.css` (`--docs-accent`, `--docs-gradient-*`, `--docs-callout-*`, `--docs-card-*`, per-group `--docs-group-*` hues; `:root` + `.dark`). Re-theming = editing that one file; never hard-code an accent in docs components or prose CSS.
- **Layout**: three-column (grouped sidebar / article / scroll-spy TOC), sticky header, ⌘K search over `lib/registry.ts`, light/dark/system toggle (reads the app's `appearance` localStorage key when served same-origin, so in-app docs match the user's app theme).
- **Content pattern**: TSX pages composed from the kit in `docs-site/components/docs/` — `DocArticle`, `Cards`/`Card`, `Steps`/`Step`, `Screenshot` (theme-aware light/dark pairs from `public/docs/images/screenshots/`), `Callout` (note/tip/important/warning/caution = blue/emerald/violet/amber/red), `CodeBlock` (build-time Shiki dual themes, copy button), `H2`/`H3` (auto anchor ids). Visual-first writing: cards for navigation, steps for procedures, tables for reference, screenshots for orientation — not walls of paragraphs.
- The in-app sidebar footer links to `/docs` as a plain full-page anchor (the docs are a static site outside Inertia).
