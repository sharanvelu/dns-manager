...
| `dns_entry_zone_provider` | **Assignment + sync state** (`EntrySyncState` pivot: entry × zone attachment) | `dns_entry_id` + `zone_provider_id` (unique pair), `external_id` (provider-side identifier), `sync_status` (`pending`\|`synced`\|`drifted`\|`error`\|`deleting`), `last_synced_at`, `last_error` |
...
- **Custom events** via the `activity()` helper: `delete-requested` on entries when deletion is deferred to queued provider-cleanup jobs (attributes the requesting user; the trait's `deleted` event fires later, system-caused), `providers-changed` on bulk reassignment (property: assigned provider names), `provider-attached` / `attachment-updated` / `provider-detached` on zones (`ZoneProviderController` — provider name only, never attachment config values), `zone-access-granted` / `zone-access-updated` / `zone-access-revoked` on zone grants (`ZoneAccessController` — user name, zone name, roles), and `login` / `logout` in `Auth\OidcController` (log name `auth`).
