...
- CI: `.github/workflows/build.yml` → GHCR on master merge (`latest`, `sha-*`, semver on `v*` tags). Docs site builds separately (`docs-site` workflow/image). `tests.yml` (Pest on sqlite `:memory:`) and `lint.yml` (Pint/Prettier/ESLint/tsc/Vite build, check-only) run on master pushes and PRs. All four use only the automatic `GITHUB_TOKEN` — no repo secrets or variables required.
