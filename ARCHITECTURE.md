...
- **External triggers**: `POST /api/hooks/drift-check` and `POST /api/hooks/health-check` (routes/api.php — no session/CSRF) queue the same checks, optionally for one `provider_id`. Guarded by `AuthenticateTriggerToken` middleware comparing the bearer token to `HOOKS_TRIGGER_TOKEN`; 404 while unset, 401 on mismatch. Built for N8N/cron-style automation.
