...
| `dns_entry_zone_provider` | **Assignment + sync state** (`EntrySyncState` pivot: entry × zone attachment) | `dns_entry_id` + `zone_provider_id` (unique pair), `external_id` (provider-side identifier — `text`, since Technitium ids embed full rData like 2 KB TXT values), `sync_status` (`pending`\|`synced`\|`drifted`\|`error`\|`deleting`), `last_synced_at`, `last_error`, `drift_details` (json list of `{field, tracked, actual}` diffs recorded by the drift check; null when not drifted or when the record is missing remotely) |
...
- `capabilities()` — `supportsProxied` / `supportsTtl` / `supportsPriority` / `supportsZones` / TTL bounds / `defaultTtl`. Drives UI field visibility, drift comparison (`RemoteRecord::matches()`/`diff()` only compare fields the connector supports; null and `defaultTtl` count as the same TTL on both sides), auto-attachment, and how drift checks iterate (see below).
...
- **Drift semantics** (`CheckProviderDrift`): compares states via `RemoteRecord::matches()` and marks `drifted`/`synced`, refreshing provider health. A drifted state also stores `drift_details` — the `RemoteRecord::diff()` field list (`{field, tracked, actual}`) against the remote record with the state's external id, falling back to the remote record at the same name+type when the id no longer matches (tuple ids change with the record's data); null when the record is missing remotely. Details are cleared on every transition to `pending`/`synced` and surface in the drifted chip's tooltip. **Zoned** connectors (`supportsZones`) loop the provider's **enabled attachments** — one zone-scoped `listRecords()` per attachment, a failing zone is isolated (logged per zone, others still checked) and health names the failing zones. **Zoneless** connectors (Pi-hole) do ONE instance-wide `listRecords()` matched against the states of every attached zone (FQDN-based external ids keep it unambiguous). States are compared with `entry.zone` eager-loaded — `RemoteRecord::matches()` compares against the FQDN.
...
- **Conflict policy**: the app DB wins — drift is flagged, and re-push ("Sync now") overwrites the remote. `zones.sync-drifted` (the Drifted stat tile's **Sync** button, `ZoneController::syncDrifted`) re-queues only the zone's drifted states, each to the one attachment it drifted on — skipping inactive attachments (paused invariant).
