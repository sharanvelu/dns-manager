...
- **Retention**: `clean_after_days` = 365 in `config/activitylog.php`; pruned by `php artisan activitylog:clean` (not scheduled by the app). `ACTIVITYLOG_ENABLED=false` disables logging. On-demand wipe: `php artisan dns:flush-activities [--days=N] [--force]` (`FlushActivities`; confirmation-gated, deliberately never scheduled).
