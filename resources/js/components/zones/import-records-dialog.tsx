...
interface ImportRecordsDialogProps {
    ...
    attachment: { id: number; providerName: string };
    open: boolean;
...
/**
 * Import a zone attachment's remote records into the zone. Names arrive
 * zone-relative; records outside the zone are skipped server-side and only
 * surfaced as a count.
 */
export function ImportRecordsDialog({ zone, attachment, open, onOpenChange }: ImportRecordsDialogProps) {
    ...
    useEffect(() => {
        ...
        fetch(route('zones.import.records', [zone.id, attachment.id]), {
            ...
            .then(async (response) => {
                ...
                setOutOfZoneCount(body.outOfZoneCount ?? 0);
    ...
    const submit = () => {
        ...
        setImporting(true);
        router.post(
            route('zones.import', [zone.id, attachment.id]),
            {
    ...
    const skippedNotes = [
        unmanagedCount > 0 ? `${unmanagedCount} record(s) hidden (types not managed by this provider)` : null,
        outOfZoneCount > 0
            ? `${outOfZoneCount} record${outOfZoneCount === 1 ? '' : 's'} outside this zone ${outOfZoneCount === 1 ? 'was' : 'were'} skipped`
            : null,
    ].filter(Boolean);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-2xl">
                <DialogHeader>
                    <DialogTitle>Import records from {attachment.providerName}</DialogTitle>
                    <DialogDescription>
                        Select the records to manage in <span className="font-mono">{zone.name}</span>. Existing entries are updated and linked —
                        never duplicated. Imported entries stay assigned to this provider only.
                    </DialogDescription>
                </DialogHeader>
                ...
                {loading && (
                    <div className="text-muted-foreground flex items-center justify-center gap-2 py-10 text-sm">
                        ...
                        Fetching records from {attachment.providerName}…
                    </div>
                )}
                ...
                {!loading && !error && records.length === 0 && (
                    <div className="space-y-1 py-8 text-center">
                        <p className="text-muted-foreground text-sm">No importable records found in this zone at {attachment.providerName}.</p>
                        {skippedNotes.length > 0 && <p className="text-muted-foreground text-xs">{skippedNotes.join(' · ')}</p>}
                    </div>
                )}
