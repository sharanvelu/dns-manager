...
- Technitium has **no stable record ids**: `external_id` is a canonical-JSON tuple that MUST be produced byte-identically by `createRecord()` and `listRecords()` (drift matches on string equality — key order, lowercased domain values). Null-TTL entries push the connector default 3600 and remote 3600 reads back as auto; don't "fix" either side alone. The auto↔3600 mapping means an entry with an **explicit** TTL 3600 reads back as auto — `capabilities().defaultTtl` + the normalization in `RemoteRecord::diff()` is what keeps that from flagging permanent false drift; keep all three in sync. And because tuple ids change with the record's data, a remotely edited record makes the stored id stale: re-pushing against it hits not-found → create and **duplicates the record** (multi-value types like A never conflict). `CheckProviderDrift` re-points the drifted state's `external_id` at the closest remote record (same name+type, unclaimed by sibling states) precisely so the re-push updates in place — don't remove that re-pointing.
